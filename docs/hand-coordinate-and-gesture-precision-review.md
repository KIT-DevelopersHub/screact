...
PR #26の続きでは、Androidの手検出を「1手のみ」と「2手」から選択可能にし、既定を「1手のみ」とする。1手モードはMediaPipeの検出上限自体を1へ下げ、送信前にも1手へ制限する。2手モードはPR #24で統合した最大2手通信とDesktopの2トラック処理を維持する。

通信の`hello.maxHands=2`と`interactionProfile=two_users_two_active_hands`は端末とPCの対応能力として維持し、利用モードの切替でプロトコル形式を変えない。これにより、接続中でも安全に切り替えられ、1手モードで余分な2手目が一時的に送られることを防ぐ。

デモの完成度は1手モードを基準に評価し、位置合わせ、ポインター、描画、手喪失、安全解除、画面境界を先に安定させる。2手モードは機能を削除せず、負荷と認識精度の追加検証を行う任意機能として扱う。

### 15.1 Desktop側の切替要否

Desktopには「1手のみ」「2手」の切替を重ねて実装しない。Desktopの`InputFrame`は受信した`hands`だけを`tracks`へ展開し、`MultiHandEngine`は各`trackId`を初めて受信した時だけ専用の`InteractionEngine`を生成する。骨格変換、ジェスチャー判定、オーバーレイ更新も実在するトラックだけを走査するため、Androidの1手モードで0〜1手しか送られない場合、Desktopは2手目の平滑化・ジェスチャー・描画状態を事前生成または常時処理しない。

この構造ではDesktop側へ同じ切替を追加しても通常経路のCPU・描画負荷は減らず、AndroidとDesktopの設定不一致、無視した2手目の押下・描画状態をいつ解除するかという追加の安全状態、利用者の操作箇所を増やす。PR #27ではDesktopの本番設定を追加せず、1手フレームで1トラック分だけ状態を保持する回帰テストを追加する。

将来、旧Androidや別クライアントから常に2手が送られる環境をDesktopだけで制限する要件が生じた場合は、単なるUIフィルターではなく、接続時の利用プロファイル合意と、除外トラックの即時安全解除を含む別PRとして扱う。Desktop描画が1手でもボトルネックだと計測された場合は、手数切替ではなく、骨格更新と操作更新の再描画統合、Pathの増分化、保持履歴上限を優先する。
