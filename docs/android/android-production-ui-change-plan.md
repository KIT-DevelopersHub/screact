...
| 基準コミット | `495b575` |
| 基準日 | 2026-08-05 |
| ステータス | 既存本番UIはImplemented／信頼済み自動接続・配置確認はPlanned |

本計画の既存本番UI部分は2026-08-06の実装へ反映済みである。2026-08-06に追加確定した信頼済み自動接続とPCでの配置確認は未実装であり、本計画で残課題として追跡する。

## 2. 目標

現在のデバッグ中心画面を残したまま、通常利用者向けに次の主要経路を追加する。

> カメラ許可 → 自動接続または初回ペアリング → スマホ固定 → PCで配置OK → ArUco位置合わせ → 操作可能 → 手の追跡 → 切断からの復旧
...
| アプリ再起動後の接続 | host・portだけ復元し、6桁コードと接続操作が必要 | `resumeToken`をKeystoreで保護し、起動時に入力なしで自動接続 |
| スマホ配置 | 位置合わせモード移行後に直ちにマーカー探索 | スマホ固定案内とPCの`配置OK`を待ち、マーカー未表示中は正常待機 |
...
1. 後方互換のため`set_mode=tracking`を最終成功通知として扱い、任意の`calibration_status`で処理中と再試行理由を補う。
2. 操作中もカメラプレビューを常時表示する。
3. debugビルドだけ本番／デバッグを切り替え、releaseは本番固定とする。
4. Androidからの再位置合わせ要求は初期版に含めない。

本番の視覚表現は正式ブランド決定まで無彩色Material 3とし、debug状態ラボと実通信モックの両方を検証環境とする。
...
- `hello`の`pairingToken`と`resumeToken`を排他的に扱い、`hello_ack.resumeToken`を受信する。
- Android Keystoreでトークンを保護し、host・portと組にして起動時接続へ使用する。
- `resume_token_invalid`では保存トークンを削除し、初回接続画面へ戻す。
- `接続先を変更`と`このPCを忘れる`で保存情報を消去する。

### Phase 6: PC配置確認とデバッグ同等経路

- `calibrationRequired=true`の直後に配置待ちUIを表示し、マーカー未表示の`0/4`をエラーにしない。
- PC担当は`配置OK`後にマーカーを表示し、PC起動ごとに位置合わせ状態を初期化する。
- モックPCと本番状態ラボへ、自動接続、配置待ち、マーカー認識、PC計算中、追跡開始を追加する。
- 実機とモックPCで初回接続、アプリ再起動、一時通信断、PC再起動、無効トークンを完走する。
