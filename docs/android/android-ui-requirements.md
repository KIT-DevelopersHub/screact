...
デバッグUIはdebugビルドで既定、本番UIはreleaseビルドで既定とする。設定から切り替えられる場合も、現在の接続や撮影を予告なく破棄しない。
...
| ID | 論点 | UIへの影響 |
| --- | --- | --- |
| UI-TBD-001 | PCから位置合わせ成功・失敗理由を返す通信形式 | 位置合わせ結果と再試行文言 |
| UI-TBD-002 | Androidから再位置合わせを要求できるか | 本番設定の再位置合わせ操作 |
| UI-TBD-003 | 操作中にカメラプレビューを常時表示するか | バッテリー、視認性、プライバシー感 |
| UI-TBD-004 | releaseビルドでデバッグUI切替を許可するか | 設定画面と誤操作防止 |
| UI-TBD-005 | アプリ正式名称、ロゴ、ブランドカラー | ビジュアルデザイン全般 |
| UI-TBD-006 | QRコードまたはPC自動検出を初期版へ含めるか | 接続画面の情報設計 |
