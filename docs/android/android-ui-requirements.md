...
flowchart TD
    ...
    SystemPermission -->|許可| Saved{信頼済みPCがあるか}
    SystemPermission -->|拒否| PermissionError[権限の復旧案内]
    Permission -->|はい| Saved
    Saved -->|はい| AutoConnecting[前回のPCへ自動接続]
    Saved -->|いいえ| Connect[PC接続画面]
    Connect --> Connecting[初回接続中]
    AutoConnecting -->|成功・位置合わせ必要| Placement[スマホを固定]
    AutoConnecting -->|成功・位置合わせ不要| Ready[操作できます]
    AutoConnecting -->|保存情報が無効| Connect
    Connecting -->|成功・位置合わせ必要| Placement
    Connecting -->|成功・位置合わせ不要| Ready
    Connecting -->|失敗| ConnectError[接続エラー]
    Placement --> PcConfirm[PCで配置OKを押す]
    PcConfirm --> Calibration[位置合わせ]
...
Androidアプリの新規起動時も、保存済みのホスト、ポート、再接続用トークンがある場合は同じ再接続表示を使用して自動接続する。利用者は自動接続をキャンセルして接続設定を変更できる。
...
| 表示条件 | 初回利用、保存済みPCがない場合、保存認証が無効な場合、利用者が接続先変更を選んだ場合 |
| 初期値 | 接続先変更時だけ前回のホストとポートを復元。6桁コードは空欄 |
| 主要操作 | `接続する` |
...
保存済みの信頼済みPCがある通常起動では、入力フォームを経由せず、権限確認後に直接「前回のPCに接続しています」へ進む。
...
自動接続中は見出しを`前回のPCに接続しています`とし、保存済みホストとポートを表示する。補助操作として`キャンセル`と`接続先を変更`を提供する。

### UI-04 位置合わせ

| 状態 | 見出し | 表示内容 |
| --- | --- | --- |
| 配置待ち | `スマホを固定してください` | PC画面全体が映る位置に固定し、PCで`配置OK`を押す案内 |
| 準備 | `PC画面全体をカメラに入れてください` | PCに4つのマーカーが表示されたこと、端末を動かさないこと |
...
PCが`配置OK`を受けるまではマーカー未表示を正常状態として扱い、`検出済み 0/4`をエラー表示しない。
...
- 操作する手の数。既定は`1手のみ`とし、負荷増加の注意を示したうえで`2手`へ切り替え可能とする
- 接続先の変更
- このPCを忘れる
- カメラ権限の状態
- セットアップ・設置方法
...
- 当面はMediaPipe解析の安定性を優先し、画面方向を一方向の横画面に固定すること。
...
- UI-AC-013: 初回接続成功後の次回起動では、接続フォームを経由せず前回のPCへ自動接続する。
- UI-AC-014: 配置待ち、PCでの配置確認、マーカー認識、PC確認中が別状態として識別できる。
- UI-AC-015: 無効な保存済みトークンでは自動接続を繰り返さず、理由を示して初回接続画面へ戻る。
