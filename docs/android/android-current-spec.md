...
要件定義書では、初回ペアリング後の入力なし自動接続と、PC上の`配置OK`を起点にする位置合わせを目標としている。現行Android実装はまだ次の差分を持つため、本書では実装済みとして扱わない。

| 目標要件 | 現行実装 | 後続実装 |
| --- | --- | --- |
| 2回目以降は入力なしで自動接続 | hostとportを復元するだけで、接続操作と6桁コードが毎回必要 | `resumeToken`の受信・Keystore保護・起動時接続・無効時フォールバック |
| PCでスマホ配置を確認後にマーカー表示 | AndroidはPCの位置合わせ指示を受けると直ちにマーカー探索状態へ入る | 配置待ちUIを追加し、マーカー未表示の`0/4`を正常待機として扱う |
| PCアプリ起動ごとに再位置合わせ | モックPCのプロセス内状態に依存 | 実PCが起動時に位置合わせ状態を初期化し、`calibrationRequired=true`を返す |
| 本番と同じフローをデバッグ環境で再現 | 既存`production-happy`は接続入力から位置合わせへ進む | 自動接続と配置確認ゲートをモック・状態ラボへ追加 |

Android側で既に実装済みのArUco安定化、21点骨格送信、再接続、疑似入力は後続フローでも再利用する。
...
flowchart TB
    Screen[メイン画面]
    Screen --> Production[本番画面]
    Screen --> Debug[デバッグ画面]
    Production --> Preview[常時Previewと簡略Overlay]
    Production --> Guide[最優先状態と主要操作]
    Debug --> Detail[21点・ArUco ID・fps・手動モード]
...
| 領域 | 表示・操作 |
| --- | --- |
| 本番カメラ状態 | 権限案内、起動中、準備完了、復旧操作付きエラー。fpsや推論時間は表示しない |
| 本番接続状態 | 未接続、接続中、接続済み、毎秒更新する再接続秒数、利用者向けエラー |
| 本番撮影モード | PC指示に従う位置合わせまたは手追跡。手動切替は表示しない |
| 接続入力 | PCのホスト、1〜65535のポート、6桁数字のペアリングコード |
| 本番操作 | 接続、キャンセル、切断、即時再接続、接続設定変更、権限復旧、ヘルプ |
| デバッグ | 手動モード、解像度、fps、しきい値、疑似入力、JSONL、本番状態ラボ |
...
- Androidのデフォルトネットワーク消失を監視し、Wi-Fiなどのネットワークが復帰した時点で待機中のソケットを破棄して即時再接続する。
- WebSocket Upgrade中または`hello_ack`待ちのどちらも5秒でタイムアウトさせ、接続試行が無期限に残らないようにする。
- PC確認済みの位置合わせ座標は同じ自動再接続の間だけメモリへ保持する。PCが再度位置合わせを要求した場合も保存座標を再送し、カメラはArUco検出へ戻さず手追跡を継続する。
...
| 最大送信fps | 20 | 5〜20 | する |
| Experience Mode | debug APKはDebug、release APKはProduction | debugだけ切替可能 | する |
...
デバッグ画面の「診断」では端末・カメラ・検出・ArUco・通信の最新値、カウンター、直近500イベント、JSONL保存、疑似21点、疑似未検出、疑似4マーカーを利用できる。「本番状態ラボ」は実カメラと実通信を変更せず本番画面の全状態を表示する。本番モードでは診断UIと詳細なしきい値を隠し、releaseからデバッグへ切り替えられない。
...
- 要件で定義した`resumeToken`の受信・保存・送信は未実装である。
...
- Androidのバックグラウンド復帰を含む実機操作。画面回転は横画面固定中のため対象外とする。
...
- アプリ新規起動時の自動接続は未実装であり、保存済みhost・portがあっても6桁コード入力と接続操作が必要である。
- PCでの配置確認待ち状態は未実装であり、位置合わせモードでは直ちにArUco探索を開始する。
- 手がカメラ画像の外へ出ると、MediaPipeがx／yの`0.0`〜`1.0`範囲外座標を返すことがある。診断では生値を保持し、送信値のみクランプする。
- マーカー安定後も安定結果が到着するたび最大5 fpsで送信する。PCからの受領確認はない。
- デバッグAPKはMediaPipeモデルとOpenCVネイティブライブラリを含むユニバーサルAPKであり、サイズが大きい。
- `hello_error`の非再試行エラーは`ERROR`接続状態として本番UIへ表示する。通信不能と一時的なサーバー混雑は再接続状態へ移る。
