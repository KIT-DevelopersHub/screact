...
| port | 数値かつ1〜65535 |
| pairingToken | 6桁の数字 |
| resumeToken | 空でない不透明文字列。pairingTokenと排他的 |
...
- PC確認済みの位置合わせ座標は同じ自動再接続の間だけメモリへ保持する。PCが再度位置合わせを要求した場合は保存座標を再送し、PCの`set_mode=tracking`を待って手追跡へ戻る。
...
| 信頼済みPC host | 空 | 空でない文字列 | resumeToken発行成功時に保存 |
| 信頼済みPC port | 8080 | 1〜65535 | resumeToken発行成功時に保存 |
| resumeToken | 空 | PC発行の不透明文字列 | Keystoreの非エクスポートAES鍵で暗号化して保存 |
...
- deviceId、信頼済みhost・port、暗号化済みresumeToken、検出設定、デバッグモードはSharedPreferencesへ保存する。
- `resumeToken`はAndroid Keystoreの非エクスポートAES/GCM鍵で保護し、値をログへ出さない。
