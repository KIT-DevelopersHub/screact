...
// UDP:8766でdiscovery_offerを受信し、responseを返した直後にAndroid側から
// PCのWebSocket:8765へ接続する現行フローを、実機なしで演じる。
//
// 使い方:
//   dart run tool/mock_pairing_android.dart [--discovery-port=18766] \
//     [--devices=2] [--respond-only]
//  - --devices=N: N台すべてがofferへ応答しWS接続を試す。PCはfirst socket winsで
//                 1台だけ受理し、後着端末をhello_error(server_busy)で拒否する
//  - --respond-only: discovery_responseだけ返し、WSへは接続しない
//  - --calib-delay=S: 四隅送信開始をS秒遅らせる（ArUco表示の目視/撮影用）
//  - discovery_select/select_ackは旧クライアント互換としてACKだけ返す
import 'dart:async';
import 'dart:convert';
import 'dart:io';
...
Future<void> main(List<String> args) async {
  ...
  stdout.writeln(
    'mock listening: udp 0.0.0.0:$discoveryPort devices=$devices '
  ...
  sock.listen((e) {
    ...
    switch (j['messageType']) {
      case 'discovery_offer':
        ...
        // 現行Androidと同じく、初回offerだけでPCのWSへ自発接続する。
        // N台モードでは全端末が接続を試し、PC側のfirst-socket-winsを検証する。
        if (respondOnly || offerHandled) return;
        offerHandled = true;
        stdout.writeln(
          'offer received → $devices device(s) connecting to WS '
          '(token=${j['token']} wsPort=${j['wsPort']})',
        );
        for (var i = 0; i < devices; i++) {
          final deviceId = 'mock-${String.fromCharCode(97 + i)}';
          unawaited(
            Future<void>.delayed(
...
/// offer受領後の実機相当ふるまい: WS接続→hello(token)→キャリブ四隅送信→
...
Future<void> _connectAndDrive({
  ...
  try {
    ws = await WebSocket.connect('ws://$host:$port/ws/v1/input');
  } catch (error) {
    stdout.writeln('$deviceId ws connect failed: $error');
    return;
  }
  stdout.writeln('$deviceId ws connected: ws://$host:$port/ws/v1/input');
        ...
        'markers': [
          {
            'id': 10,
            'center': v(0.1, 0.1),
            'corners': [v(.08, .08), v(.12, .08), v(.12, .12), v(.08, .12)],
          },
          {
            'id': 11,
            'center': v(0.9, 0.1),
            'corners': [v(.88, .08), v(.92, .08), v(.92, .12), v(.88, .12)],
          },
          {
            'id': 12,
            'center': v(0.9, 0.9),
            'corners': [v(.88, .88), v(.92, .88), v(.92, .92), v(.88, .92)],
          },
          {
            'id': 13,
            'center': v(0.1, 0.9),
            'corners': [v(.08, .88), v(.12, .88), v(.12, .92), v(.08, .92)],
          },
        ],
      });
    });
    ...
    (data) {
      ...
      switch (j['messageType']) {
        case 'hello_ack':
          ...
          stdout.writeln(
            '$deviceId hello_ack session=$sessionId '
        ...
        case 'control_message':
          stdout.writeln(
            '$deviceId control: ${j['command']} ${j['mode'] ?? ''}',
          );
          ...
          break;
        case 'hello_error':
          stdout.writeln('$deviceId hello_error: ${j['code']}');
          break;
      }
    },
    onDone: () {
      calibTimer?.cancel();
      handTimer?.cancel();
      stdout.writeln('$deviceId ws closed');
    },
  );
