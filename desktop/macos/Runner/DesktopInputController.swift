...
final class DesktopInputController: NSObject {
  ...
  private func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "setOverlayVisible":
      ...
      // OverlayWindowController が担当する。この共通DesktopBridge呼び出しは
      // 互換用no-opとして成功させ、MissingPlugin扱いで入力backendが無効に
      // ならないようにする。
      result(nil)
    case "applyEvent":
  ...
  private func point(_ args: [String: Any]) -> CGPoint {
    ...
    let x = bounds.minX + min(max(nx, 0), 1) * bounds.width
    let y = bounds.minY + min(max(ny, 0), 1) * bounds.height
  ...
  private func apply(_ args: [String: Any]) {
    ...
    case "click":
      ...
      break
    case "scroll":
      let size = mainDisplayBounds().size
