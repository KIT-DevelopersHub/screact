...
void main() {
  group('GestureRecognizer', () {
    test('2本指くっつきを検出し、drawPointは人差し指と中指の中間点', () {
      ...
      final f = MockHand.at(
          frameId: 0, tip: const Vec2(0.5, 0.5), pinch: false, together: true);
    ...
    test('指が離れていれば fingersTogether は false', () {
      final rec = GestureRecognizer();
      final f = MockHand.at(
          frameId: 0, tip: const Vec2(0.5, 0.5), pinch: false, together: false);
    ...
    test('ピンチとくっつきは独立に判定される（ピンチ時は描画しない）', () {
      final rec = GestureRecognizer();
      final f = MockHand.at(
          frameId: 0, tip: const Vec2(0.5, 0.5), pinch: true, together: false);
      final pose = rec.recognize(f)!;
    ...
    test('くっつきのヒステリシス（境界のチャタリング防止）', () {
      ...
      final rec = GestureRecognizer(togetherOnRatio: 0.35, togetherOffRatio: 0.55);
      // まず確実にくっつける（ratio小）→ together=true
      rec.recognize(MockHand.at(
          frameId: 0, tip: const Vec2(0.5, 0.5), pinch: false, together: true));
      // 離す（ratio大）→ together=false
      final off = rec.recognize(MockHand.at(
          frameId: 1, tip: const Vec2(0.5, 0.5), pinch: false, together: false));
  ...
  group('InteractionEngine ジェスチャー分岐', () {
    ...
    test('くっつきを解くと drawUp が出る', () {
      final e = calibrated();
      e.onFrame(MockHand.at(
          frameId: 0, tip: const Vec2(0.5, 0.5), pinch: false, together: true));
      final ev = e.onFrame(MockHand.at(
          frameId: 1, tip: const Vec2(0.5, 0.5), pinch: false, together: false));
    ...
    test('つまむ（ピンチ）で pressDown→（離すと）click＋pressUp・drawは出ない', () {
      final e = calibrated();
      final down = e.onFrame(MockHand.at(
          frameId: 0, tip: const Vec2(0.5, 0.5), pinch: true, together: false));
      ...
      final up = e.onFrame(MockHand.at(
          frameId: 1, tip: const Vec2(0.5, 0.5), pinch: false, together: false));
    ...
    test('描画中の筆点は中間点（人差し指先端と一致しない）', () {
      ...
      for (var i = 0; i < 3; i++) {
        for (final ev in e.onFrame(MockHand.at(
            frameId: i, tip: const Vec2(0.5, 0.5), pinch: false, together: true))) {
